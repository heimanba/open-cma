...
describe("buildSessionBindings", () => {
  ...
  test("throws when agent not found in config", () => {
    ...
    const state = makeState();
    expect(() =>
      buildSessionBindings("nonexistent", config, "qoder", state)
  ...
  test("throws when agent has no environment and none specified", () => {
    ...
    const state = makeState();
    expect(() =>
      buildSessionBindings("no-env", config, "qoder", state)
    ).toThrow(/no environment declared/);
  });
  ...
  test("throws when agent not provisioned (not in state)", () => {
    ...
    const state = makeState();
    expect(() =>
      buildSessionBindings("unprovisioned", config, "qoder", state)
...
describe("resolveSessionProvider", () => {
  ...
  test("throws for multi-provider with no override and no agent provider", () => {
    ...
    expect(() => resolveSessionProvider("researcher", config)).toThrow(
      /multiple providers/i
    );
  });

  test("throws when agent not in config", () => {
    const config = makeConfig();
    expect(() => resolveSessionProvider("ghost", config)).toThrow(
      /not found in config/
    );
  });
});
