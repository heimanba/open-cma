...
本文档记录 OpenCMA 状态管理的核心设计决策：为什么保留 state file、为什么不做 metadata-based discovery、如何精简到最小负担。
...
2. **Metadata 标记**：在远程资源上写入 `OpenCMA.resource=researcher` 标记，用 list + filter 找回 → 要求所有资源类型都支持 metadata 写入

OpenCMA 的结论是：**两种替代方案都无法统一实施，state file 仍然必要，但应精简到最小负担。**
...
- **serial**：Terraform 用于 state locking 和并发控制。OpenCMA 单用户本地运行，从不读取
- **lineage**：Terraform 用于检测 state 文件是否被替换。OpenCMA 从不读取
...
虽然 metadata 覆盖率不足以替代 state file，但对支持 metadata 的资源类型，仍然注入 `OpenCMA.*` 标记。这是一个预留动作，不用于 discovery，但提供了平台侧的可追溯性。
...
注入的 key：
- `OpenCMA.project`：从 config 文件所在目录名推导
- `OpenCMA.resource`：资源在 YAML 中的声明名
...
|------|------------------|--------|
| **平台侧可追溯** | ✅ 可在平台 UI 看到哪些资源由 OpenCMA 管理 | ❌ 无法区分 |
...
State refresh（从远程状态重建本地 state）和 state import（将已有远程资源导入 state）是 Terraform 的重要功能。OpenCMA 选择暂不实现。
...
| **OpenCMA** | 本地文件 | 4 字段精简映射 | ❌（metadata 仅标记） | ❌（暂不实现） |

### 关键洞察

1. **Terraform/Pulumi 存了太多**：完整 attributes 快照用于 drift detection 和 plan 输出，但 OpenCMA 的变更检测基于本地 YAML hash，不需要远程快照。

2. **Kubernetes/Serverless FW 没有 state file**：它们的 state 在服务端，工具直接查询。OpenCMA 管理的平台不是自建服务，无法控制服务端存储。

3. **OpenCMA 取了最小集**：只存必须的映射关系，不存远程快照。这是 Terraform 和 Kubernetes 之间的一个有意为之的中间位置。
...
OpenCMA 的变更检测：
  上次部署的 YAML hash（content_hash in state）  vs  当前 YAML hash
  → 只能检测本地变更，不能检测 drift
```

这个差异是有意的设计选择：OpenCMA 管理的资源数量少（通常 <20），drift detection 的投入产出比不高。用户如果怀疑远程状态不一致，可以 `destroy + apply` 重建。
